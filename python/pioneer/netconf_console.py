...
class OutputThread(threading.Thread):
    def __init__(self, iocb, fd):
        super(OutputThread, self).__init__()
        self.iocb = iocb
        self.fd = fd

    def run(self):
        buf = self.fd.read(bufsiz)
        while buf:
            self.iocb.output(buf)
            buf = self.fd.read(bufsiz)
...
class NetconfSSHLikeTransport(object):
    ...
    def create_connection(iocb, host, port):
...
class NetconfSSH(NetconfSSHLikeTransport):
    ...
    def connect(self):
        ...
        self.ssh = self.paramiko.Transport(sock)

        if self.publicKeyType == 'rsa':
            agent_public_key = self.paramiko.RSAKey(
                data=base64.decodestring(self.publicKey))
        elif self.publicKeyType == 'dss':
            agent_public_key = self.paramiko.DSSKey(
                data=base64.decodestring(self.publicKey))
        else:
        ...
        if not self.privateKeyFile == '':
            if self.privateKeyType == "rsa":
                user_private_key = self.paramiko.RSAKey.from_private_key_file(self.privateKeyFile)
            #elif self.privateKeyType == "dss":
            else:
                user_private_key = self.paramiko.DSSKey.from_private_key_file(self.privateKeyFile)
            ...
            try:
                ...
                self.iocb.abort("Authentication failed.")
...
class NetconfTCP(NetconfSSHLikeTransport):
    ...
    def __init__(self, iocb,
                 hostname, port, username, groups, suplgids):
        NetconfSSHLikeTransport.__init__(self, iocb)
        self.hostname = str(hostname)
        self.port = int(port)
        self.username = username
...
def main(sys_args, iocb, logger=None):
    print "netconf_console %s"%sys_args
    if logger:
    ...
    if filename:
        ...
        ##JANLmsg = get_msg("get", None, o.xpath, o.wdefaults, o.winactive)
    ...
    while not is_closed and (n != 0 or o.interactive):
        ...
        if do_print and o.style in ("pretty", "all", "noaaa"):
            if use_subprocess:
                p = subprocess.Popen(xmllint, shell=True,
        ...
        while True:
            ...
            if code < 0:
                ...
                if code == -1:
                    iocb.abort("unexpected EOF in NETCONF transport")
                if code == -2:
                    iocb.abort("NETCONF transport framing error")
            nchunks += 1
